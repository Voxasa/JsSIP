...
module.exports = class RTCSession extends EventEmitter
{
  ...
  onTransportError()
  {
    ...
    if (this._status !== C.STATUS_TERMINATED)
    {
      // For confirmed sessions, don't terminate immediately - give ICE recovery a chance
      // The WebSocket may reconnect and ICE renegotiation can recover the call
      if (this._status === C.STATUS_CONFIRMED || this._is_confirmed)
      {
        logger.debug('Transport error during confirmed session - emitting event for recovery');
        this.emit('transporterror');
        // Don't terminate - wait for ICE state to determine call fate
        return;
      }

      // For non-confirmed sessions (call setup phase), terminate immediately
      // There's no active media path to preserve
      this.terminate({
        status_code   : 500,
  ...
  _createRTCConnection(pcConfig, rtcConstraints)
  {
    ...
    this._connection.addEventListener('iceconnectionstatechange', () =>
    {
      ...
      // Emit event for app to handle all ICE state changes
      ...
      {
        // Emit event for app to handle reconnection
        // Don't terminate - give the app a chance to recover via ICE restart
        logger.debug('ICE disconnected - emitting event for recovery');
